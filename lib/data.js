...
export async function getProperties() {
  try {
    ...
    return properties.map((prop) => ({
      ...
      // The typeSlug should ideally come from the database, but we can generate it if needed
      typeSlug: `coworking-${createSlug(prop.type)}`,
    }));
  } catch (error) {
...
// NEW FUNCTION: Fetches all location data (cities and localities) from the API
export async function getLocations() {
  try {
    ...
    const res = await fetch(`${baseUrl}/api/locations`, {
      next: { tags: ["locations"] }, // Add a tag for on-demand revalidation
    });
